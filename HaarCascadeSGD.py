...
# then it also predicts whether the face is male or female.
    ...
    def face_cascade_start(self):
        ...
        # load classifier
        self.clf = joblib.load(f'faceclassifier.joblib')
        ...
        if self.isRect:
            for files in os.listdir(current_path):
                ...
                self.data['filename'].append("rulersmall"+str(i)+".jpg")
                self.data['data'].append(im)
                i+=1
    ...
    def predict_gender(self):
        ...
        scalify = joblib.load(f'jobscalifier.pkl')
        grayify = joblib.load(f'jobgrayifier.pkl')
        hogify = joblib.load(f'jobhogifier.pkl')
        ...
        if self.isRect:
            ...
            y_pred = self.clf.predict(X_test_prepared)
